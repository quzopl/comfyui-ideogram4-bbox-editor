# Ideogram4 Bbox Editor Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** A ComfyUI custom node that renders the Ideogram-4 bbox/caption editor on the node and outputs the assembled caption as a JSON string.

**Architecture:** A minimal v1 Python node (`Ideogram4BboxEditor`) exposes one multiline string widget `caption_json` and returns it (validated) as `STRING prompt`. A frontend extension (`web/ideogram4_bbox_editor.js`) hides that widget and renders the full editor as a DOM widget, writing `buildCaption()` JSON into the widget on every edit. All build logic lives in JS (a verbatim port of `ideogram4_bbox_editor(1).html`), plus an added caption-level fields section.

**Tech Stack:** Python 3 (ComfyUI custom node, v1 `INPUT_TYPES`), vanilla JS (ComfyUI/LiteGraph `addDOMWidget`), pytest.
...
comfyui-ideogram4-bbox-editor/
  ...
  nodes.py           # Ideogram4BboxEditor (v1): caption_json widget -> STRING prompt
  web/
    ideogram4_bbox_editor.js   # ported editor as a DOM widget
  tests/
    test_node.py     # unit tests for build()
  README.md
  .gitignore
```

Responsibilities:
- `nodes.py` — thin pass-through: parse + re-serialize the JSON, never rebuilds it.
- `web/ideogram4_bbox_editor.js` — all interactive logic + caption assembly.
- `tests/test_node.py` — covers the Python validation only (UI is manual-tested).

---

## Task 1: Python node scaffold + validation (TDD)

**Files:**
- Create: `comfyui-ideogram4-bbox-editor/nodes.py`
- Create: `comfyui-ideogram4-bbox-editor/__init__.py`
- Test: `comfyui-ideogram4-bbox-editor/tests/test_node.py`

- [ ] **Step 1: Write the failing test**

Create `comfyui-ideogram4-bbox-editor/tests/test_node.py`:
...
Run: `cd comfyui-ideogram4-bbox-editor && python -m pytest tests/test_node.py -v`
Expected: FAIL — `nodes.py` does not exist / `Ideogram4BboxEditor` not defined.
...
```bash
cd comfyui-ideogram4-bbox-editor
git add nodes.py __init__.py tests/test_node.py
git commit -m "feat: Ideogram4BboxEditor node + JSON validation"
```

---

## Task 2: Frontend editor (DOM widget)

**Files:**
- Create: `comfyui-ideogram4-bbox-editor/web/ideogram4_bbox_editor.js`

Reference (port source, logic preserved verbatim): `/home/bart/Pobrane/ideogram4_bbox_editor(1).html`.

- [ ] **Step 1: Create the JS extension**

Create `comfyui-ideogram4-bbox-editor/web/ideogram4_bbox_editor.js` with this exact content:
...
- [ ] **Step 2: Manual test in ComfyUI**

1. Restart ComfyUI (so the new custom node + web dir load).
2. Add node **Ideogram4 Bbox Editor** (category `Ideogram4`).
3. Verify: editor renders on the node; one `obj` box present; the hidden `caption_json` widget is NOT visible.
4. Draw/move/resize boxes; switch AR (4:5/16:9/1:1); add `text` element; edit desc/text/palette (swatches update); reorder with ▲/▼; delete.
5. Click "Paste JSON", paste the sample below, Load — boxes + caption fields populate:

```json
{"high_level_description":"A cinematic portrait of quz0","style_description":{"aesthetics":"cinematic","lighting":"soft","photo":"85mm f/1.8","medium":"photograph","color_palette":["#2A2520","#E8E2D8"]},"compositional_deconstruction":{"background":"urban bokeh","elements":[{"type":"obj","bbox":[40,240,1000,760],"desc":"quz0 athletic man"}]}}
```

6. Connect the node's `prompt` output to a Show-Text / CLIPTextEncode `text`; queue; confirm the output JSON equals the editor's live preview (`pre`).
7. Save the workflow, reload the page, reopen — confirm the editor restores the boxes/fields from the saved `caption_json`.

- [ ] **Step 3: Commit**

```bash
cd comfyui-ideogram4-bbox-editor
git add web/ideogram4_bbox_editor.js
git commit -m "feat: on-node bbox/caption editor (DOM widget)"
```
...
**Files:**
- Create: `comfyui-ideogram4-bbox-editor/README.md`
- Create: `comfyui-ideogram4-bbox-editor/.gitignore`
...
- **Spec coverage:** node (Task 1), DOM-widget editor with all ported logic +
  caption-level fields (Task 2), STRING-only output (Task 1 `RETURN_TYPES`),
  import via editor (Task 2), distribution/README (Task 3). All spec sections
  covered.
- **Types/names consistent:** node id `Ideogram4BboxEditor`, widget
  `caption_json`, output `prompt`, JS class match `nodeData.name`, function
  names (`buildCaption`, `serialize`, `fitFrame`, `loadCaption`) used
  consistently.
- **No placeholders:** all code is complete; the only manual step is the
  inherently-manual ComfyUI UI test.
```
